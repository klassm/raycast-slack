...
export async function cachedConversations(
	...
	const cached = await getCachedData<ConversationCache>(
		cacheKey,
		async () => ({}),
		{
	...
	const foundConversations = pickBy(cached, (value) =>
		conversationIds.includes(value.id),
	);

	const missingConversations = conversationIds.filter(
	...
	const newlyLoadedConversations = await loadConversationInfos(
		credentials,
		missingConversations,
	);
	const indexedConversations = keyBy(
		newlyLoadedConversations,
		(user) => user.id,
	);
