...
export function UnreadConversations() {
  const { data, loading } = useClientCounts();
  ...
  const renderGroup = (team: TeamInfo, results: SlackEntryWithUnread[]) => {
    return (
      <List.Section title={ team.name } key={ team.id }>
        { results.map((result) => (
          <ConversationItem key={ result.id } conversation={ result } team={ team }/>
        )) }
      </List.Section>
    );
  };
...
function ConversationItem({ conversation, team }: { conversation: SlackEntryWithUnread; team: TeamInfo }) {
...
function messageToMarkdown(message: MessageWithUser): string {
  ...
  const content = compact([message.text, files, ...attachments]).filter(it => it).join("\n");
  return header + "\n" + content;
}
