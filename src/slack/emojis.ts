...
export async function loadEmojis({
	...
	if (!isEmojiResponse(json)) {
		...
		throw new Error(
			`Got a weird emoji response from Slack: ${response.status} ${response.statusText}`,
		);
	}
...
export async function loadEmojisCached(
	credentials: Credentials,
	teamId: string,
): Promise<Emojis> {
	return getCachedData<Emojis>(
		`slack-emojis-list-${teamId}`,
		async () => loadEmojis(credentials),
		{
