...
async function loadPaged(
	...
	if (!isConversationResponse(json)) {
		console.log(
			"Got a weird conversation response from Slack",
			json,
			response.status,
		);
		throw new Error(
			`Got a weird conversation response from Slack: ${response.status} ${response.statusText}`,
		);
	}

	const nextCursor = json.response_metadata.next_cursor;
	const pageData = nextCursor
		? await loadPaged({ cookie, token }, nextCursor)
		: [];
...
): Promise<{ [id: string]: ConversationChannel }> {
