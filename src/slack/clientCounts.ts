...
function isClientCountsResponse(response: unknown): response is ClientCountResponse {
  const casted = response as ClientCountResponse;
  return casted.ok && casted.threads && Array.isArray(casted.channels) && Array.isArray(casted.mpims) && Array.isArray(casted.ims);
}
...
export async function clientCounts({ cookie, token }: Credentials): Promise<ClientCount> {
  ...
  if (!isClientCountsResponse(json)) {
    console.log("Got a weird client counts response from Slack", json, response.status);
    throw new Error(`Got a weird client counts response from Slack: ${ response.status } ${ response.statusText }`);
  }
