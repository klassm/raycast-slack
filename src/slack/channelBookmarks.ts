...
export async function channelsBookmarks(
	{ cookie, token }: Credentials,
	channelId: string,
): Promise<ChannelBookmark[]> {
	const response = await fetch(
		`https://slack.com/api/bookmarks.list?channel_id=${channelId}`,
		{
			method: "POST",
	...
	if (!isBookmarksResponse(result)) {
		...
		throw new Error(
			`Got a weird bookmarks response from Slack: ${response.status} ${response.statusText}`,
		);
	}
