...
function isChannelResponseEntry(value: unknown): value is ChannelResponseEntry {
	...
	return (
		...
		entry.context_team_id !== undefined
	);
}
...
function isUserResponseEntry(value: unknown): value is UserResponseEntry {
	...
	return (
		entry.id !== undefined &&
		entry.profile.real_name !== undefined &&
		entry.team_id !== undefined
	);
}

function isSearchResponse<T>(
	value: unknown,
	entryVerifier: (entry: unknown) => boolean,
): value is SearchResponse<T> {
	const response = value as SearchResponse<T>;
	return (
		response.ok !== undefined &&
		Array.isArray(response.results) &&
		response.results.every(entryVerifier)
	);
}
...
async function searchChannels(
	...
	const response = await fetch(
		"https://edgeapi.slack.com/cache/T0ATUH6S1/channels/search",
		{
			method: "POST",
	...
	if (!isSearchResponse<ChannelResponseEntry>(result, isChannelResponseEntry)) {
		...
		throw new Error(
			`Got a weird search channels response from Slack: ${response.status} ${response.statusText}`,
		);
	}
...
async function searchUsers(
	...
	const response = await fetch(
		"https://edgeapi.slack.com/cache/T0ATUH6S1/users/search",
		{
			method: "POST",
			headers: { Cookie: cookie, "Content-Type": "application/json" },
			body: JSON.stringify({
	...
	if (!isSearchResponse<UserResponseEntry>(result, isUserResponseEntry)) {
		console.log(
			"Weird search users response from Slack",
			result,
			response.status,
		);
		throw new Error(
			`Got a weird search users response from Slack: ${response.status} ${response.statusText}`,
		);
	}
...
export async function search(
	...
	const results = await Promise.all([
		searchUsers(credentials, query),
		searchChannels(credentials, query),
	]);
	return results.flat();
}
