...
): conversation is UserConversation {
...
function isConversationInfoResponse(
	response: unknown,
...
async function loadConversation(
	...
	if (!isConversationInfoResponse(json)) {
		console.log(
			"Got a weird conversation info response from Slack",
			json,
			response.status,
		);
		throw new Error(
...
export async function loadConversationInfos(
	...
	return Promise.all(
		conversationIds.map(async (userId) =>
			loadConversation(credentials, userId),
		),
	);
}
